...
// Convert matrix from row major to 2D array
Matrix<float> convertRowMajorToMatrix(float *arr, int n, int m){
    Matrix<float> matrix(n, m);
    for (int i = 0; i < n; i++){
        for (int j = 0; j < m; j++){
...
// Takes input result after applying FC1 [250x144] -> RELU -> FC2 [144x144] -> RELU -> FC3 [144X144] -> RELU -> FC4 [144x12] -> softmax, audioFile and sets pred as top 3 possibilities 
...
// Applies FC1 [250x144] -> RELU -> FC2 [144x144] -> RELU -> FC3 [144X144] -> RELU -> FC4 [144x12] -> softmax and returns
